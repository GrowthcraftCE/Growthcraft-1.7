    ...
    public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random random) {
        recipeList.add(
            new MerchantRecipe(
                ...
                GrowthCraftApples.fluids.appleCider.asStack(1, 1)));
        recipeList.add(
            new MerchantRecipe(
                new ItemStack(Items.emerald, 2 + random.nextInt(2)),
                GrowthCraftApples.fluids.appleCider.asStack(1, 2)));
        recipeList.add(
            new MerchantRecipe(
                new ItemStack(Items.emerald, 2 + random.nextInt(2)),
                GrowthCraftApples.fluids.appleCider.asStack(1, 3)));
    }

    @Override
    public PieceWeight getVillagePieceWeight(Random random, int i) {
        int num = MathHelper.getRandomIntegerInRange(random, i, 1 + i);
        if (!generateAppleFarms) num = 0;
