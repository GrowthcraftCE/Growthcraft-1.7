    ...
    public void manipulateTradesForVillager(EntityVillager villager, MerchantRecipeList recipeList, Random random) {
        recipeList.add(
                new MerchantRecipe(
                        ...
                        GrowthCraftHops.fluids.hopAle.asStack(1, 1)));
        recipeList.add(
                new MerchantRecipe(
                        new ItemStack(Items.emerald, 2 + random.nextInt(2)),
                        GrowthCraftHops.fluids.hopAle.asStack(1, 2)));
        recipeList.add(
                new MerchantRecipe(
                        new ItemStack(Items.emerald, 2 + random.nextInt(2)),
                        GrowthCraftHops.fluids.hopAle.asStack(1, 3)));
    }

    @Override
    public PieceWeight getVillagePieceWeight(Random random, int i) {
        int num = MathHelper.getRandomIntegerInRange(random, i, 1 + i);
        if (!GrowthCraftHops.getConfig().generateHopVineyardStructure) num = 0;
